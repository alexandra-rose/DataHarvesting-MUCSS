...
```{r}
trial2$movietitle <- gsub("\\d+\\.\\s*", "", trial2$movietitle)
trial2$movietitle <- trimws(trial2$movietitle)
```

```{r}
trial2 <- na.omit(trial2)
trial2 <- trial2[nchar(trimws(trial2$all_comments)) > 0, ]
```
...
first_three_movies <- trial2 %>% 
  filter(movietitle %in% unique(trial2$movietitle)[1:3])
